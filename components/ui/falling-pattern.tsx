...
// NOTE: This project uses plain CSS (no Tailwind), so the original component's
// Tailwind utility classes (`relative h-full w-full p-1`, `size-full`, `z-0`,
// `absolute inset-0 z-1`, `dark:brightness-600`) have been converted to inline
// styles. The visual pattern (radial-gradient definitions + framer-motion
// background-position animation) is otherwise unchanged. `dark:brightness-600`
// is intentionally dropped for a subtler look; add `filter: 'brightness(6)'` to
// the overlay `<div>` below if you want the original punchier dark-mode effect.
export function FallingPattern({
    ...
    return (
        <div
            ...
            style={{ position: 'relative', height: '100%', width: '100%', padding: '0.25rem' }}
        >
            <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
